...
RUN apt update -y && apt upgrade -y && apt install -y wget git \
    ...
    tk8.6-dev python3-tk python3-pip libharfbuzz-dev libfribidi-dev \
    libxcb1-dev libfftw3-dev && rm -rf /var/lib/apt/lists/*

# 創建符號連結將 python 指向 python3
...
RUN cd /usr/local/DocsaidKit && python setup.py bdist_wheel && \
    python -m pip install dist/*.whl &&  rm -rf /usr/local/DocsaidKit
