...
def test_imresize():
    ...
    resized_img = imresize(img, (200, 400), INTER.BILINEAR)
    assert resized_img.shape == (200, 400, 3)

    # resize to a smaller size
    resized_img = imresize(img, (100, 50), INTER.BILINEAR)
    ...
    resized_img, w_scale, h_scale = imresize(
        img, (200, 400), INTER.BILINEAR, return_scale=True)
    assert resized_img.shape == (200, 400, 3)
    assert w_scale == 2
    assert h_scale == 2
    ...
    resized_img = imresize(img, (200, 400), "BILINEAR")
    assert resized_img.shape == (200, 400, 3)
...
def test_imwarp_quadrangle():
    # 使用隨機生成的四個點來進行透視變換
    src_pts = np.array([[10, 10], [90, 10], [90, 90],
...
def test_imwarp_quadrangles_invalid_input():
    ...
    with pytest.raises(TypeError):
        polygons = [Polygon(np.array([[10, 10], [90, 10], [90, 90], [
                            10, 90]], dtype=np.float32)), "invalid_polygon"]
        imwarp_quadrangles(img, polygons)
